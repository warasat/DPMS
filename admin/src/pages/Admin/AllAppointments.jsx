    ...
    <div className='w-full max-w-6xl m-5'>
      ...
      <div className='bg-white border rounded-lg shadow-lg text-sm max-h-[80vh] min-h-[60vh] overflow-y-scroll'>
        ...
        {appointments.map((item, index) => (
          <div className='flex flex-wrap justify-between sm:grid sm:grid-cols-[0.5fr_3fr_1fr_3fr_3fr_1fr_1fr] items-center text-gray-500 py-4 px-6 border-b hover:bg-gray-50 transition-all duration-200' key={index}>
            ...
            <div className='flex items-center gap-3 sm:items-start'>
              <img className='w-10 h-10 rounded-full object-cover' src={item.userData.image} alt="Patient Avatar" />
              <p className='text-gray-700 font-medium'>{item.userData.name}</p>
            </div>
  
            {/* Age */}
            <p className='text-gray-600 py-2'>{calculateAge(item.userData.dob)}</p>
  
            {/* Date & Time */}
            <p className='text-gray-600 py-2'>{slotDateFormat(item.slotDate)} , {item.slotTime}</p>
            
            {/* Doctor Info */}
            <div className='flex items-center gap-3 sm:items-start'>
              <img className='w-10 h-10 rounded-full object-cover' src={item.docData.image} alt="Doctor Avatar" />
              <p className='text-gray-700 font-medium'>{item.docData.name}</p>
            </div>
            
            {/* Fees */}
            <p className='text-gray-600 py-2'>{currency}{item.amount}</p>
